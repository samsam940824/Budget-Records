...
export default function App() {
  ...
  const handleSaveBudget = () => {
    ...
    const budgetData = {
      id: editingBudgetId || generateId(),
      amount: Number(newBudgetAmount),
      category_id: newBudgetCatId,
      repeat: newBudgetRepeat,
      end: newBudgetEnd,
      endDate: newBudgetEnd ? newBudgetEndDate : null,
      note: newBudgetNote
    };

    if (editingBudgetId) {
      setBudgets(budgets.map(b => b.id === editingBudgetId ? budgetData : b));
    } else {
  ...
  const handleEditBudget = (budget: any) => {
    ...
    setNewBudgetRepeat(budget.repeat);
    setNewBudgetEnd(budget.end || false);
  ...
  return (
    <div className="min-h-screen bg-black font-sans text-zinc-100 selection:bg-emerald-500/30">
      ...
      {selectedBudget && (
        <div className="fixed inset-0 bg-black z-50 flex flex-col animate-in slide-in-from-bottom-10">
          <div className="flex justify-between items-center px-4 pt-8 pb-4">
            <button onClick={() => setSelectedBudget(null)} className="bg-zinc-800 text-zinc-300 p-2 rounded-full">
              <X size={20} />
            </button>
